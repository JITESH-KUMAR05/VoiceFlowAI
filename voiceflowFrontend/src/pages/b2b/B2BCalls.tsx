...
import { b2bCalls } from "@/data/mockData";
...
export default function B2BCalls() {
  ...
  const filteredCalls = b2bCalls.filter((call) => {
  ...
  const selectedCallData = b2bCalls.find((call) => call.id === selectedCall);
            ...
            <div className="space-y-6">
              ...
              {selectedCallData.summary && (
                <div className="glass-card p-4">
                  ...
                  <p className="text-sm text-muted-foreground">{selectedCallData.summary}</p>
                </div>
              )}

              {selectedCallData.objections && selectedCallData.objections.length > 0 && (
                <div className="glass-card p-4">
                  <h4 className="font-medium text-foreground mb-2">Objections Detected</h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedCallData.objections.map((obj, i) => (
                      <Badge key={i} className="bg-yellow-500/20 text-yellow-500">{obj}</Badge>
                    ))}
                  </div>
                </div>
              )}
